...
Ext.define('Ext.ux.form.ItemSelector', {
    ...
    /**
     * This is set to true so that we can tell if an empty
     * fromStore indicates that the store has not been
     * loaded OR if all items have just been removed from it.
     * There appears to be a bug in the default code in which
     * if all items are removed from the fromStore, the component
     * setValue method stops working, b/c it thinks the fromStore
     * just hasn't loaded yet. derp
     *
     * @private
     */
    isLoaded : false,
     ...
     */
    buttonsText: {
        top: "#{msgs.button_move_to_top}",
        up: "#{msgs.button_move_up}",
        add: "#{msgs.button_add_to_selected}",
        remove: "#{msgs.button_remove_from_selected}",
        down: "#{msgs.button_move_down}",
        bottom: "#{msgs.button_move_to_bottom}"
    },
        ...
        if (!me.hideNavIcons) {
            Ext.Array.forEach(me.buttons, function(name) {
                buttons.push({
                    ...
                    cls: Ext.baseCSSPrefix + 'form-itemselector-btn secondaryBtn',
    ...
    setValue: function(value){
        ...
        if (!this.isLoaded && !me.fromField.store.getCount()) {
            ...
            // sailpoint - see isloaded for more on this.
            this.isLoaded=true;
        }
